...
///
/// MUST be a public top-level function and MUST be registered from `main()`
/// before `runApp()` — see `main.dart`. It previously lived inside
/// `initialise()`, behind a permission check that ran after the first frame,
/// so background data messages arriving before that point reached no handler
/// at all.
...
class PushNotificationService {
  ...
  Future<void> initialise() async {
    ...
    // The background handler is registered in main() — it must be in place
    // before the first frame, not here.

    // Listeners are attached UNCONDITIONALLY, whatever the permission answer.
    // Previously everything below sat inside an `authorized` check, so a user
    // who declined and later enabled notifications in OS settings got messages
    // in the tray that did nothing when tapped, permanently. With no permission
    // nothing is delivered, so these listeners simply never fire — harmless.
    ...
    // 6. Token management. Only the token write is gated on authorization —
    //    an unauthorised device has no useful token to register.
  ...
  static bool isAuthorized(AuthorizationStatus status) =>
      status == AuthorizationStatus.authorized ||
      status == AuthorizationStatus.provisional;

  /// Re-checks the OS permission and registers the token if it is now granted.
  ///
  /// Call this when the app resumes: a user who declined at first launch and
  /// later enabled notifications in Settings would otherwise never have a token
  /// written, so the server could never reach them.
  Future<void> refreshPermissionState() async {
    try {
      ...
      AppLog.error('PushNotificationService.refreshPermissionState', e);
    }
  }
  ...
  Future<void> syncTokenAfterLogin() async {
    // refreshPermissionState covers the "declined at first launch, enabled in
    // Settings later" case; it no-ops when permission is still absent.
  ...
  void _handleNotificationClick(RemoteMessage message) {
    ...
    if (type == 'request_accepted') {
      ...
      // Broadcasts carry no routing data (empty `data` map) — land on the
      // recipient's OWN home, so a volunteer isn't dumped on the user home.
      final isVolunteer =
          locator<AuthenticationService>().currentUser?.isVolunteer ?? false;
      navigationService.clearStackAndShow(
          isVolunteer ? Routes.volunteerHomeView : Routes.homeView);
    }
  }
}
